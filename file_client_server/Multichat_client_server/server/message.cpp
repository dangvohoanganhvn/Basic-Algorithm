...
bool Process_Msg::Encapsulate(Message *msg, unsigned char *buffer)
{
    ...
    unsigned char len_byte[4]={0}; //length field wich contain the total length of msg
    memcpy(len_byte, &len, 4);
    m_count_number_send=std::ceil(len/sizeof(buffer)) ;
    //copy src id
    uint16_t src_id=msg->m_src_id;
    unsigned char src_id_byte[2]={0};
    memcpy(src_id_byte,&src_id, 2);
    int number_send;
    //copy des id
    uint16_t des_id=msg->m_des_id;
    unsigned char des_id_byte[2]={0};
    memcpy(des_id_byte,&des_id, 2);
    if(m_count_number_send==1)
    {
        number_send=len;
    }
