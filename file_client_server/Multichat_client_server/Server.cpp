...
int Server::Recieve_Msg(int socket){
    ...
    /* if(remain_stt==false)
    {
        ...
        if( (m_msg_rcv->data.length()+9)< m_msg_rcv->m_len)
        {
            remain_stt=true;
        }
    }
    else {
        std::string tmp(reinterpret_cast<char*>(buffer_rcv));
        m_msg_rcv->data+=tmp;
        if( (m_msg_rcv->data.length()+9)==m_msg_rcv->m_len)
        {
            remain_stt=false;
    ...
    if(m_msg_rcv->m_type==CHAT)
    {
        ...
        std::cout << "Client " << m_msg_rcv->m_src_id<<" to Client "<<m_msg_rcv->m_des_id<< std::endl;
    }
    */
            ...
            if (FD_ISSET(i, &read_fds)) { // we got one!!
                if (i == m_socket)
                {
                    ...
                    else
                    {
                        ...
                        } while (m_list_clients.find(m_id_max_client)!=m_list_clients.end()); //generate id for new client with ramdomed id
*/
                        m_id_max_client++;
                        m_list_clients.insert(std::pair<int,int>(m_id_max_client,m_client_socket));
                        m_list_clients_socket.insert(std::pair<int,int>(m_client_socket,m_id_max_client));
                        std::cout<<"Add new client with id "<<m_list_clients_socket[m_client_socket]<<std::endl;
                        ...
                        SetDataSend(id,AUTHEN,0,m_id_max_client);
                ...
                else
                {
                    ...
                    if (byte_rcv<= 0) {
                        ...
                        if(m_msg_rcv->m_type==CHAT)
                        {
                            std::cout << "Client " << m_list_clients_socket[i]<<" to Client "<<m_list_clients_socket[i]+1<<": "<<m_msg_rcv->data<< std::endl;
                        }
                        else {
                            std::cout << "Client " << m_list_clients_socket[i]<<" to Client "<<m_list_clients_socket[i]+1<< std::endl;
                        }

                        int total = 0;

                        int len=byte_rcv;// how many bytes we've sent
                        int bytesleft = len; // how many we have left to send
                        int n;

                        while(total < len)
                        {
                            n = send(m_list_clients[m_list_clients_socket[i]+1], buffer_rcv+total, bytesleft, 0);
                            if (n == -1)
                            {

                                std::cout<<m_list_clients_socket[i]<<" to "<<m_list_clients_socket[i]+1<<": "<< "cannot send total file or message: "<<bytesleft<<std::endl;
                        ...
                        //SetDataSend(m_msg_rcv->data,m_msg_rcv->m_type,m_list_clients_socket[i],m_list_clients_socket[i+1]);
                        //this->Send_Msg(i);
                        ...
                        int total = 0;
                        int len=strlen((char*)buffer_rcv);// how many bytes we've sent
                        int bytesleft = len; // how many we have left to send
                        int n;
                        std::cout<<" Send to "<<it->second<<std::endl;
                                    while(total < len)
                        {
                                    n = send(it->first, buffer_rcv+total, bytesleft, 0);
                                    if (n == -1) { break; }
                                    total += n;
                                    bytesleft -= n;
                    } */
                        /* if(m_msg_rcv->m_type==CHAT)
                                        {
                                            SetDataSend(m_msg_rcv->data,m_msg_rcv->m_type,m_msg_rcv->m_src_id,m_msg_rcv->m_des_id);
                                        }
                                        else if(m_msg_rcv->m_type==FILE)
                                        {
                                            SetDataSend(m_msg_rcv->data,m_msg_rcv->m_type,m_msg_rcv->m_src_id,m_msg_rcv->m_src_id+1);
                                        }
                                            ...
                                            if(remain_stt==false)
                                            {
                                                for(int j = 0; j <= fdmax; j++)
                                                {
                                                    ...
                                                    if (FD_ISSET(j, &master))
                                                    {
                                                        ...
                                                        if (j != m_socket && j != i&&j==m_list_clients[m_msg_rcv->m_src_id+1])
                                                        {
                                                            this->Send_Msg(j);
                                                        }
                                                    }
